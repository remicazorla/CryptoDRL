...
PATH_DATA = "data/usdt_data/"
PATH_SYMBOL = "data/symbols.csv"
PATH_BAD_SYMBOL = "data/badSymbols.csv"
NBR_VALUE_FEATURE = 7000
client = Client()
...
def get_pair_tickers_database(sort_binance = True):
    try:
        symbols = []
...
def save_symbol(symbols = get_pair_tickers_database()):
    try:
...
def update_data_features(symbols = get_pair_tickers_database()):
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for f in concurrent.futures.as_completed(results):
            ...
            try:
                f.result()
                print(f"Update Symbol : {symbol} Done")
...
def update_data(symbols = get_pair_tickers_database()):
...
def save_data_pickle(df, symbol):
    if isinstance(df, pd.DataFrame):
        if(date.today() - timedelta(days=1) <= df.index[-1]):
            try:
            ...
            print(f"Crypto Morte Date dépassée {symbol} derniere date : {df.index[-1]}")
            return False
